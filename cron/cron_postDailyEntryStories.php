...
while($obj = mysql_fetch_object($rs)) {
	...
	$rs2 = $db->doQuery("SELECT strategies.goal_id FROM strategies_log INNER JOIN strategies ON strategies.id=strategies_log.strategy_id WHERE strategies_log.user_id=%s AND strategies_log.entered_at_day=%s", $userID, $today);
	$obj2 = null;
	while($obj2 = mysql_fetch_object($rs2)) {
